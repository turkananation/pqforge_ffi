...
(Uint8List, Uint8List) _nativeFromSeed(
  _SeedKeygenDart fn,
  PqSignatureAlgorithm alg,
  Uint8List seed,
) {
  final s = calloc<Uint8>(seed.length)
...
void main() {
  ...
  if (!File(libPath).existsSync()) {
    test('ML-DSA KATs', () {}, skip: 'native library not built at $libPath');
    return;
  }
  ...
  for (final alg in PqSignatureAlgorithm.values) {
    ...
    test(
      ...
      () {
        final seed = Uint8List.fromList([
          for (var i = 0; i < 32; i++) (i * 5 + 2) & 0xFF,
        ]);
        final (nativePub, nativeSec) = _nativeFromSeed(
          nativeSeedKeygen,
          alg,
          seed,
        );
        ...
        expect(
          nativePub,
          equals(pqKp.publicKey),
          reason:
              'two independent FIPS-204 KeyGen(ξ) must agree on the public key',
        );
        expect(
          nativeSec,
          equals(pqKp.secretKey),
