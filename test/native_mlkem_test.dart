...
typedef _KeygenNative = Int32 Function(Int32, Pointer<Uint8>, Size, Pointer<Size>,
    Pointer<Uint8>, Size, Pointer<Size>);
typedef _KeygenDart = int Function(int, Pointer<Uint8>, int, Pointer<Size>,
...
typedef _DecapNative = Int32 Function(Int32, Pointer<Uint8>, Size, Pointer<Uint8>,
    Size, Pointer<Uint8>, Size, Pointer<Size>);
typedef _DecapDart = int Function(int, Pointer<Uint8>, int, Pointer<Uint8>, int,
    Pointer<Uint8>, int, Pointer<Size>);
...
class _NativeKem {
  _NativeKem(DynamicLibrary lib)
      : _keygen =
            lib.lookupFunction<_KeygenNative, _KeygenDart>('pqforge_mlkem_keygen'),
        _encap = lib
            .lookupFunction<_EncapNative, _EncapDart>('pqforge_mlkem_encapsulate'),
        _decap = lib
            .lookupFunction<_DecapNative, _DecapDart>('pqforge_mlkem_decapsulate');
  ...
  (Uint8List, Uint8List) keygen(PqKemAlgorithm alg) {
    ...
    try {
      _check(_keygen(alg.index, pk, alg.publicKeyBytes, pl, sk,
      ...
      calloc..free(pk)..free(sk)..free(pl)..free(sl);
    }
  }
  ...
  (Uint8List, Uint8List) encapsulate(PqKemAlgorithm alg, Uint8List publicKey) {
    ...
    final sl = calloc<Size>();
    try {
      _check(_encap(alg.index, pub, publicKey.length, ct, alg.ciphertextBytes,
          cl, ss, alg.sharedSecretBytes, sl));
      return (
  ...
  Uint8List decapsulate(
      PqKemAlgorithm alg, Uint8List secretKey, Uint8List ciphertext) {
    final sk = _toNative(secretKey);
    final ct = _toNative(ciphertext);
    final ss = calloc<Uint8>(alg.sharedSecretBytes);
    final sl = calloc<Size>();
    try {
      _check(_decap(alg.index, sk, secretKey.length, ct, ciphertext.length, ss,
          alg.sharedSecretBytes, sl));
      return Uint8List.fromList(ss.asTypedList(sl.value));
    } finally {
      calloc..free(sk)..free(ct)..free(ss)..free(sl);
    }
  }

  void _check(int rc) {
    if (rc != PqForgeErrorCode.ok.value) {
      throw StateError('native KEM failed: ${PqForgeErrorCode.fromValue(rc).name}');
    }
  }
}
...
void main() {
  ...
  for (final alg in PqKemAlgorithm.values) {
    test('${alg.name}: native keygen → encapsulate → decapsulate round-trip', () {
    ...
    test('${alg.name}: cross-backend — native key, pqforge encapsulate, native decapsulate',
        () {
      final (pk, sk) = native.keygen(alg);
    ...
    test('${alg.name}: cross-backend — pqforge key, native encapsulate, pqforge decapsulate',
        () {
